...
class Vtiger
{
    ...
    public function __construct()
    {
        ...
        $this->url               = config( 'vtiger.vtiger_url' );
        $this->username          = config( 'vtiger.vtiger_username' );
        $this->accessKey         = config( 'vtiger.vtiger_key' );
        $this->persistConnection = config( 'vtiger.vtiger_persistent' );
        $this->maxRetries        = config( 'vtiger.vtiger_retries' );
    ...
    public function addRelated($sourceRecordId,$relatedRecordId)
    {
        $sessionId = $this->sessionId();
    ...
    /**
     *
     * @param string $crmLoginUserId
     * @param string $casinoUserId
     * @param string $gameId
     * @param string $shopId
     * @param string $userIPAdds
     * @param string $spinSymbols json data
     * @param array $gameState
     */
    public function gameHistory(string $crmLoginUserId,string $casinoUserId,string $gameId,string $shopId,string $userIPAdds,string $spinSymbols,$gameState=array()) {
        $spinSymbolsObj = json_decode($spinSymbols);
        $responseEvent = $spinSymbolsObj->responseEvent;
        $responseType = $spinSymbolsObj->responseType;
        $openBalance = $spinSymbolsObj->serverResponse->Balance;
        $afterBalance = $spinSymbolsObj->serverResponse->afterBalance;
        $slotBetVolumn = $gameState['bet'];
        $slotTotalWin = $spinSymbolsObj->serverResponse->totalWin;
        $createData = array(
            "trading_id" => $crmLoginUserId,
            "login" => $gameState['game'],
            "order_id" => $shopId,
            "symbol" => $responseEvent,
            "type" => $responseType,
            "comment"=>$userIPAdds,
            "game_logs"=>$spinSymbols,
            "volume" => $slotBetVolumn,"assigned_user_id"=>$this->userId(),"source"=>"Casino","open_price"=>$openBalance,"close_price"=>$afterBalance,"profit"=>$slotTotalWin
        );
        $res = $this->create("Tradinghistory", $createData);
        if($res->success){
            $tradehistoryId = $res->result->id;
            $resR = $this->addRelated($crmLoginUserId,$tradehistoryId);
        }
    }
}
